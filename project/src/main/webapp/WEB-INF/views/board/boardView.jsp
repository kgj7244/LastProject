		...
		<table class="table table-striped">
			...
			<tr>
				<td colspan="8"><a href="boardList.do?pageNum=${pageNum }"
					...
					<c:if test="${board.member_id == member_id || member_id == 'master'}">
						<a
						href="boardUpdateForm.do?b_num=${board.b_num }&pageNum=${pageNum }"
						class="btn btn-warning">수정</a> <a
						href="boardDelete.do?b_num=${board.b_num }&pageNum=${pageNum }"
			...
			<c:if test="${member.member_id == 'master' }">
				...
				<table class="table table-hover">
					<tr>
						<td>작성자</td>
						<td><input type="text" name="member_id" value="${member.member_id }" size="4" readonly="readonly"></td>
						<td>댓글</td>
						<td><textarea rows="2" cols="100" name="r_text"></textarea></td>
						<td><input type="button" value="입력" id="reInsert" class="btn btn-danger btn-sm"></td>
					</tr>
				</table>
			</c:if>
		</form>
